...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I,printf("dest:%ld\n",(int64_t) (signed)Mr(src1 + src2, 4)); R(dest) =(int64_t) (signed)  Mr(src1 + src2, 4));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu    , B, if(src1<src2){printf("src1:%d<src2:%d\n",(signed)src1,(signed)src2);s->dnpc=dest+s->pc;}else printf("src1:%d>src2:%d\n",(signed)src1,(signed)src2));
