...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) =src1 & src2);  
              //0011000 00101 01110 001 00000 11100 11
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs   , I,R(dest) =Cr(src2);Cw(src2,Cr(src2)|src1););
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw   , I,R(dest) =Cr(src2);Cw(src2,src1););//printf("src2:%lx--src1:%lx\n",src2,src1);R(dest) =Cr(src2);Cw(src2,src1););  
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 0110011", srl    , R, R(dest) =((uint64_t) src1) >> SEXTU(BITS(src2, 5, 0), 6));  
  ...
  INSTPAT("0011000 00010 00000 000 00000 1110011", mret    , R, s->dnpc=Cr(MEPC_INDEX));  
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall , N, s->dnpc=isa_raise_intr(0xb, s->pc));//printf("pc:0x%lx\n",s->dnpc)); // 按照difftest应设置0xb,按照nanos-lite应设置成1
