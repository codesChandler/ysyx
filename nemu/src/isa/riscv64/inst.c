...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) =((int64_t) src1) >> SEXTU(BITS(src2, 4, 0), 5) );
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(dest) =((uint64_t) src1) << SEXTU(BITS(src2, 4, 0), 5) );  
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) =((uint64_t) src1) >> SEXTU(BITS(src2, 4, 0), 5) );  
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 0110011", slu     , R, if(((int64_t)src1) < ((int64_t)src2)) R(dest) = 1;else R(dest) = 0);  
  INSTPAT("0000000 ????? ????? 011 ????? 0110011", sltu    , R, if(((uint64_t)src1) < ((uint64_t)src2)) R(dest) = 1;else R(dest) = 0);  
